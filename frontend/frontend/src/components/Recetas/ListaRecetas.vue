<template><div>
    <b-card header-tag="header" footer-tag="footer">
        ...
        <div v-if="recetas.length>0">
            <b-table
                :items="recetas" :fields="campos" :per-page="per_page" :current-page="current_page" :busy.sync="is_busy" head-variant="dark"
                fixed striped hover small
                @row-clicked="myRowClickHandler()"
            >
                <template #cell(accion)="data">
                    <b-button size="sm" variant="primary" :to="{name:'CrearEditarReceta', params: {crear_editar:'editar', prefill_uuid: data.item.uuid, prefill_paciente_uuid: '_'}}">Editar</b-button>
