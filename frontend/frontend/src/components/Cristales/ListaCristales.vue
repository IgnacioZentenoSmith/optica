<template><div>
    <b-card header-tag="header" footer-tag="footer">
        ...
        <!-- CONTENIDO: TABLA -->
        <div v-if="cristales.length>0">
            <b-table
                :items="cristales" :fields="campos" :per-page="per_page" :current-page="current_page" :busy.sync="is_busy" head-variant="dark"
                fixed striped hover small
                @row-clicked="myRowClickHandler()"
            >
                <template #cell(accion)="data">
                    <b-button size="sm" variant="primary" :to="{name:'CrearEditarCristal', params: {crear_editar:'editar', prefill_uuid: data.item.uuid}}">Editar</b-button>
                    <b-button size="sm" variant="primary" :to="{name:'RangosCristal', params: {prefill_uuid: data.item.uuid}}">Rangos</b-button>
    ...
    methods: {
        buscar(){
            ...
            if(this.busqueda.length < 3) {
                ...
                this.getCristales(this.busqueda)
            }
        },
    },
    mounted() {
